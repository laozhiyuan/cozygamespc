(function () {
        ...
        <form class="form-grid" id="siteSettingsForm">
          ...
          <div class="form-grid two">
            <div class="field">
              ...
              <input id="siteIconImage" name="iconFile" type="file" accept="image/*" data-preview-target="siteIconPreview" data-hidden-target="siteIconImageData">
            </div>
        ...
        <form class="form-grid" id="categoryForm">
          ...
          <div class="form-grid two">
            <div class="field">
              ...
              <input id="categoryIconImage" name="iconFile" type="file" accept="image/*" data-preview-target="categoryIconPreview" data-hidden-target="categoryIconImageData">
        ...
        <form class="form-grid" id="gameForm">
          ...
          <div class="form-grid two">
            <div class="field">
              ...
              <input id="gameCoverImage" name="coverFile" type="file" accept="image/*" data-preview-target="gameCoverPreview" data-hidden-target="gameCoverImageData">
            </div>
  ...
  function renderBulkImportCard() {
    ...
    const matchedImages = rows.filter((row) => pendingBulkImages.has(normalizeImportNo(row.no))).length;

    return `
      <div class="admin-card">
        <h2>Bulk Import Games</h2>
        <p class="admin-card-note">Upload a CSV or Excel file with these columns: No, Title, PRELOADER Link, Category, Description, Tags, Instructions. Existing game titles are marked red and skipped, so original games are never replaced. After reading the file, upload cover images named with the matching No, such as 1.png or 001.jpg.</p>
        <div class="form-grid two">
          <div class="field">
          ...
          rows.length
            ? `
              <div class="bulk-import-summary">
                ...
                <span>${matchedImages} cover image${matchedImages === 1 ? "" : "s"} matched</span>
              </div>
  ...
  function renderBulkPreviewRow(row) {
    const image = pendingBulkImages.get(normalizeImportNo(row.no));
    return `
  ...
  async function handleImageUpload(input) {
    ...
    if (preview) preview.innerHTML = `<img src="${escapeAttr(dataUrl)}" alt="">`;
  }
  ...
  async function handleBulkImageUpload(input) {
    ...
    for (const file of files) {
      ...
      pendingBulkImages.set(no, await readCompressedImageAsDataURL(file));
      matched += 1;
    }
  ...
  function normalizeBulkGameRow(source, rowNumber) {
    ...
    const hasContent = [no, title, iframeUrl, categoryNames.join(""), description, tags.join(""), instructions].some(Boolean);
    const errors = [];
  ...
  function importBulkGames() {
    ...
    const importedGames = readyRows.map((row) => {
      ...
      return {
        ...
        coverImage: pendingBulkImages.get(normalizeImportNo(row.no)) || ""
      };
    });
  ...
  function saveSiteSettings(form) {
    ...
    const iconImage = String(data.get("iconImageData") || "").trim();
  ...
  function saveCategory(form) {
    ...
    const iconImage = String(data.get("iconImageData") || data.get("iconImage") || "").trim();
  ...
  function addGame(form) {
    ...
    const coverImage = String(data.get("coverImageData") || "").trim();
